...
import { loadBuiltinLabelsList, loadBuiltinKeyLabel } from './builtin-key-labels'
...
/**
 * Make sure the index has QWERTY entry and all built-in key labels
 * from /hub/key-labels/labelslist.json. The store now drives the
 * full row list on the modal (including the built-in entries) so that
 * a user-defined order can be persisted like any other label.
 */
async function ensureBuiltinKeyLabels(): Promise<void> {
  const labels = getStorage(STORAGE_KEYS.KEY_LABELS) || { entries: [] }

  // Load the labels list from the built-in preset directory
  const labelsListUrl = '/hub/key-labels/labelslist.json'
  let labelsList: { labels: Array<{ id: string; name: string; filename: string }> } | null = null
  try {
    const response = await fetch(labelsListUrl)
    if (response.ok) {
      labelsList = await response.json()
    }
  } catch {
    // If labelslist.json is not available, skip loading built-in presets
    console.warn('labelslist.json not available, skipping built-in key labels')
  }

  // Ensure QWERTY entry exists
  const qwertyExists = labels.entries.find((e: any) => e.id === 'qwerty')
  if (!qwertyExists) {
    const now = new Date()
    const filename = `qwerty_${now.toISOString().replace(/:/g, '-')}.json`
    ...
    const data = {
      meta,
      data: { name: 'QWERTY', map: {} }
    }

    // Pin QWERTY to the head on first creation
    labels.entries.unshift(meta)
    labels['qwerty'] = data
    setStorage(STORAGE_KEYS.KEY_LABELS, labels)
  } else {
    // Backfill uploaderName for stores created before the field was set
    if (!qwertyExists.uploaderName) {
      qwertyExists.uploaderName = 'pipette'
      setStorage(STORAGE_KEYS.KEY_LABELS, labels)
    }
  }

  // Load built-in key labels from labelslist.json
  if (labelsList && labelsList.labels) {
    for (const labelDef of labelsList.labels) {
      const exists = labels.entries.find((e: any) => e.id === labelDef.id)
      if (exists) {
        // Backfill uploaderName if missing
        if (!exists.uploaderName) {
          exists.uploaderName = 'builtin'
          setStorage(STORAGE_KEYS.KEY_LABELS, labels)
        }
        continue
      }

      // Try to load the actual key label data
      try {
        const dataResponse = await fetch(`/hub/key-labels/${labelDef.filename}`)
        if (!dataResponse.ok) {
          console.warn(`Failed to load key label ${labelDef.filename}: ${dataResponse.status}`)
          continue
        }

        const keyLabelData = await dataResponse.json() as { name: string; map: Record<string, string>; composite_labels?: Record<string, string> | null }

        const now = new Date()
        const meta: KeyLabelMeta = {
          id: labelDef.id,
          name: labelDef.name,
          uploaderName: 'builtin',
          filename: labelDef.filename,
          savedAt: now.toISOString(),
          updatedAt: now.toISOString(),
          ...(keyLabelData.composite_labels ? { compositeLabels: keyLabelData.composite_labels } : {}),
        }

        const data = {
          meta,
          data: {
            name: keyLabelData.name,
            map: keyLabelData.map,
            ...(keyLabelData.composite_labels ? { compositeLabels: keyLabelData.composite_labels } : {}),
          }
        }

        labels.entries.push(meta)
        labels[labelDef.id] = data
      } catch (err) {
        console.warn(`Failed to load key label ${labelDef.filename}:`, err)
      }
    }
...
export const vialAPI = {
  ...
  // --- Key Label Store ---
  keyLabelStoreList: async (): Promise<KeyLabelStoreResult<KeyLabelMeta[]>> => {
    await ensureBuiltinKeyLabels()
    const labels = getStorage(STORAGE_KEYS.KEY_LABELS) || { entries: [] }
    return { success: true, data: labels.entries || [] }
  },
  keyLabelStoreListAll: async (): Promise<KeyLabelStoreResult<KeyLabelMeta[]>> => {
    await ensureBuiltinKeyLabels()
    const labels = getStorage(STORAGE_KEYS.KEY_LABELS) || { entries: [] }
    return { success: true, data: labels.entries || [] }
  },
  ...
  keyLabelStoreDelete: (id: string): Promise<KeyLabelStoreResult<void>> => {
    if (id === 'qwerty') {
      return Promise.resolve({ success: false, error: 'QWERTY cannot be deleted' })
    }
    const labels = getStorage(STORAGE_KEYS.KEY_LABELS) || { entries: [] }
      ...
      input.onchange = (e: any) => {
        ...
        reader.onload = (event) => {
          try {
            ...
            if (!parsed.name || !parsed.map) {
              resolve({ success: false, error: 'Invalid key label file' })
              return
            }
            
            const labels = getStorage(STORAGE_KEYS.KEY_LABELS) || { entries: [] }
            const existing = labels.entries?.find((e: any) => 
              e.name.toLowerCase() === parsed.name.toLowerCase()
            )
            
            const id = existing?.id || crypto.randomUUID()
            const filename = `${id}_${new Date().toISOString().replace(/:/g, '-')}.json`
            ...
            const data = {
              ...
              data: {
                name: parsed.name,
                map: parsed.map,
                compositeLabels: parsed.compositeLabels || parsed.composite_labels
              }
            }
            
            if (existing) {
              const idx = labels.entries.findIndex((e: any) => e.id === id)
              if (idx !== -1) labels.entries[idx] = meta
            } else {
              labels.entries.push(meta)
            }
  ...
  keyLabelStoreExport: (id: string): Promise<KeyLabelStoreResult<{ filePath: string }>> => {
    ...
    const body = {
      ...
      map: record.data.map,
      composite_labels: record.data.compositeLabels ?? null
    }
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `${record.meta.name.replace(/[^a-zA-Z0-9_-]+/g, '_') || 'key-label'}.json`
    a.click()
    URL.revokeObjectURL(url)
    
    return Promise.resolve({ success: true, data: { filePath: a.download } })
  },
  keyLabelStoreReorder: (orderedIds: string[]): Promise<KeyLabelStoreResult<void>> => {
    const labels = getStorage(STORAGE_KEYS.KEY_LABELS) || { entries: [] }
    if (labels.entries) {
      const byId = new Map(labels.entries.map((e: any) => [e.id, e]))
      ...
      for (const entry of labels.entries) {
        if (byId.has(entry.id)) newEntries.push(entry)
      }
  ...
  keyLabelStoreSetHubPostId: (id: string, hubPostId: string | null): Promise<KeyLabelStoreResult<KeyLabelMeta>> => {
    ...
    if (entry) {
      if (hubPostId) {
        entry.hubPostId = hubPostId
      } else {
        delete entry.hubPostId
        delete entry.hubUpdatedAt
      }
      entry.updatedAt = new Date().toISOString()
      setStorage(STORAGE_KEYS.KEY_LABELS, labels)
      return Promise.resolve({ success: true, data: entry })
  ...
  keyLabelStoreHasName: (name: string, excludeId?: string): Promise<KeyLabelStoreResult<boolean>> => {
    ...
    const exists = labels.entries?.some((e: any) => 
      e.id !== excludeId && e.name.trim().toLowerCase() === target
    )
  ...
  keyLabelHubList: (params?: HubKeyLabelListParams): Promise<KeyLabelStoreResult<HubKeyLabelListResponse>> => {
    return fetchKeyLabelList(params || {})
      .then((data) => ({ success: true, data }))
      .catch((err) => ({ success: false, error: err.message || 'Hub list failed' }))
  },
  keyLabelHubDetail: (hubPostId: string): Promise<KeyLabelStoreResult<HubKeyLabelItem>> => {
    return fetchKeyLabelDetail(hubPostId)
      .then((data) => ({ success: true, data }))
      .catch((err) => ({ success: false, error: err.message || 'Hub detail failed' }))
  },
  keyLabelHubDownload: (hubPostId: string): Promise<KeyLabelStoreResult<KeyLabelMeta>> => {
    return downloadKeyLabel(hubPostId)
      .then((body) => {
        const labels = getStorage(STORAGE_KEYS.KEY_LABELS) || { entries: [] }
        ...
        const existingIdx = labels.entries?.findIndex((e: any) => e.id === hubPostId)
        if (existingIdx !== undefined && existingIdx >= 0) {
          labels.entries[existingIdx] = meta
        } else {
          labels.entries.push(meta)
        }
        
        labels[hubPostId] = record
        setStorage(STORAGE_KEYS.KEY_LABELS, labels)
        
        return { success: true, data: meta }
      })
      .catch((err) => ({ success: false, error: err.message || 'Hub download failed' }))
  },
  keyLabelHubUpload: (localId: string): Promise<KeyLabelStoreResult<KeyLabelMeta>> => {
    return Promise.resolve({ success: false, error: 'Hub upload not available in web version' })
  },
  keyLabelHubUpdate: (localId: string): Promise<KeyLabelStoreResult<KeyLabelMeta>> => {
    return Promise.resolve({ success: false, error: 'Hub update not available in web version' })
  },
  keyLabelHubSync: (localId: string): Promise<KeyLabelStoreResult<KeyLabelMeta>> => {
    return Promise.resolve({ success: false, error: 'Hub sync not available in web version' })
  },
  keyLabelHubTimestamps: (ids: string[]): Promise<KeyLabelStoreResult<HubKeyLabelTimestampsResponse>> => {
    return fetchKeyLabelTimestamps(ids)
      .then((data) => ({ success: true, data }))
      .catch((err) => ({ success: false, error: err.message || 'Hub timestamps failed' }))
  },
  keyLabelHubDelete: (localId: string): Promise<KeyLabelStoreResult<void>> => {
    return Promise.resolve({ success: false, error: 'Hub delete not available in web version' })
  },
  ...
  appConfigGetAll: (): Promise<AppConfig> => {
    const stored = getStorage(STORAGE_KEYS.APP_CONFIG) || {}
    return Promise.resolve({ ...DEFAULT_APP_CONFIG, ...stored } as AppConfig)
  },
