...
- **編集モード**: キーコードピッカーが行の下に常時表示されます。各キーコードタイルにはホバー時の **×** 削除ボタンが表示され、Tap 行には編集モードを抜ける **Close** ボタンが表示されます。アクションごとの Save ボタンはありません。ピッカー・キーコードポップオーバーでの選択は**即座にコミット**されます。選択中の位置を削除すると、編集セッションを抜けずにインデックスをずらして編集を継続します。外側クリックハンドラはピッカー・マクロアクションリスト・フッター・キーポップオーバー以外をクリックすると編集モードを抜けるので、Close ボタンを押す以外にも、モーダル背景やタイトル領域などの**外側をクリック**することでもセッションは終わります
...
- フッター (Clear / Revert / Save) は**編集モード中も非表示**です（ピッカーやポップオーバーのクリックが即コミットなので Save は不要）。リストモードに戻すとフッターが再表示されます
